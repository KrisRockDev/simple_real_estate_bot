в файле parser_cian/parser.py

```python
import os
import time
from parser_cian import func
import requests
...
def parse_cian(URL, cookies, headers):
    ...
    if status_code == 200:
        ...
        result = {
            # 'adress': func.get_adress(soup),
            # 'price': func.get_price(soup),
            # 'offer':func.get_offer(soup),
            # 'metro':func.get_metro(soup),
            # 'params':func.get_params(soup),
            'description':func.get_description(soup),
            # 'imgages': func.get_imgages(soup, cian_number),
        }
